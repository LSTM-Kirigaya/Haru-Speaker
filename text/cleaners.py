...
def japanese_to_romaji_with_accent(text):
    ...
    for i, sentence in enumerate(sentences):
        if re.match(_japanese_characters, sentence):
            ...
            print('enter japanese_to_romaji_with_accent')
            labels = sentence
...
def zh_ja_mixture_cleaners(text):
    ...
    for chinese_text in chinese_texts:
        ...
        cleaned_text = re.sub('i[aoe]', lambda x: 'y' +
                              x.group(0)[1:], cleaned_text)
        cleaned_text = re.sub('u[aoəe]', lambda x: 'w' +
                              x.group(0)[1:], cleaned_text)
        cleaned_text = re.sub('([ʦsɹ]`[⁼ʰ]?)([→↓↑]+)', lambda x: x.group(1) +
                              'ɹ`'+x.group(2), cleaned_text).replace('ɻ', 'ɹ`')
        cleaned_text = re.sub(
